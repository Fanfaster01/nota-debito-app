// src/components/cajas/PagoMovilList.tsx
...
interface PagoMovilListProps {
  pagosMovil: PagoMovilUI[]
  onEdit: (pago: PagoMovilUI) => void
  onDelete: (pagoId: string) => void
  loading?: boolean
}

export const PagoMovilList: React.FC<PagoMovilListProps> = ({
  pagosMovil,
  onEdit,
  onDelete,
  loading
  ...
  if (pagosMovil.length === 0) {
    return (
      <Card>
        <div className="text-center py-8">
          <CurrencyDollarIcon className="h-12 w-12 text-gray-400 mx-auto mb-4" />
          <p className="text-gray-500">No hay pagos móviles registrados</p>
  ...
  return (
    <>
      <Card title={`Pagos Móviles Registrados (${pagosMovil.length})`}>
            ...
            <tfoot className="bg-gray-50">
              <tr>
                ...
                <td className="px-6 py-4 text-sm text-right font-bold text-gray-900">
                  {formatMonto(pagosMovil.reduce((sum, pago) => sum + pago.monto, 0))}
                </td>
                <td></td>
              </tr>
            </tfoot>
